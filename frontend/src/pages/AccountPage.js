...
const AccountPage = () => {
  ...
  const fetchBills = async (searchDate, blNumberValue) => {
    setLoading(true);
    try {
      const params = {};
      if (searchDate) params.completed_at = searchDate.format('YYYY-MM-DD');
      if (blNumberValue) params.bl_number = blNumberValue;
      const res = await axios.get('/api/account_bills', { params });
  ...
  const handleExportPDF = () => {
    ...
    doc.setFontSize(12);
    doc.text(`Total Entries: ${summary.totalEntries || 0}`, 20, 35);
    doc.text(`Total CTN Fees: $${summary.totalCtnFee || 0}`, 20, 45);
    doc.text(`Total Service Fee: $${summary.totalServiceFee || 0}`, 20, 55);
    doc.text(`Bank Transfer: $${summary.bankTotal || 0}`, 20, 65);
    doc.text(`Allinpay 85%: $${summary.allinpay85Total || 0}`, 20, 75);
    doc.text(`Allinpay Reserve: $${summary.reserveTotal || 0}`, 20, 85);
    ...
    const tableRows = bills.map(bill => [
      bill.bl_number || '',
      `$${bill.display_ctn_fee ?? bill.ctn_fee ?? 0}`,
      `$${bill.display_service_fee ?? bill.service_fee ?? 0}`,
      `$${(
        Number(bill.display_ctn_fee ?? bill.ctn_fee ?? 0) +
        Number(bill.display_service_fee ?? bill.service_fee ?? 0)
      ).toFixed(2)}`,
      bill.customer_name || '',
      bill.payment_method || '',
  ...
  return (
    <div>
      <h2>Completed Bills - Account Page</h2>
      <div style={{ marginBottom: 16 }}>
        ...
        <Button
          type="primary"
          onClick={() => fetchBills(date, blNumber)}
          style={{ marginRight: 8 }}
        >
          Search
        </Button>
        <Button onClick={() => { setDate(null); setBlNumber(''); fetchBills(null, ''); }}>
          Clear Search
        </Button>
        <Button
          type="link"
      ...
      <div style={{ marginBottom: 16 }}>
        <b>Total Entries:</b> {summary.totalEntries || 0} &nbsp;
        <b>Total CTN Fees:</b> ${summary.totalCtnFee || 0} &nbsp;
        <b>Total Service Fee:</b> ${summary.totalServiceFee || 0} &nbsp;
        <b>Bank Transfer:</b> ${summary.bankTotal || 0} &nbsp;
        <b>Allinpay 85%:</b> ${summary.allinpay85Total || 0} &nbsp;
        <b>Allinpay Reserve:</b> ${summary.reserveTotal || 0}
      </div>
      <Table
        columns={columns}
        dataSource={bills}
