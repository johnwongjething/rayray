...
function Review({ t = x => x }) {
  ...
  const [linkGenerated, setLinkGenerated] = useState(false); // New state for button message
  ...
  const fetchBills = async (params = {}) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/bills?${query.toString()}`, {
        method: 'GET',
        credentials: 'include', // <--- This is the important addition
        headers: { 
  ...
  const showModal = (record) => {
    ...
    setLinkGenerated(false); // Reset link generated state
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/generate_payment_link/${selected.id}`, {
        ...
        body: JSON.stringify({
          ...
          ctn_fee: ctnFee, // Capture from input field
          service_fee: serviceFee, // Capture from input field
        }),
      });
      ...
      if (res.ok && data.payment_link) {
        ...
        setLinkGenerated(true); // Set flag to show message
          ...
          <div style={{ display: 'flex', gap: 24 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ marginTop: 16 }}>
                ...
                {linkGenerated ? (
                  <span style={{ color: 'green' }}>Payment Link Generated</span>
                ) : fields.payment_link ? (
