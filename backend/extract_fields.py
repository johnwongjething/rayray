...
def extract_company_info(text, start_keywords, stop_keywords, max_lines=5):
    """Extract company information between start and stop keywords"""
    lines = [line.strip() for line in text.split('\n') if line.strip()]
    collecting = False
    info_lines = []
    
    for line in lines:
        if any(re.search(rf'\b{kw}\b', line, re.IGNORECASE) for kw in start_keywords):
            collecting = True
            continue
        if collecting:
            if any(re.search(rf'\b{kw}\b', line, re.IGNORECASE) for kw in stop_keywords):
                break
            if line and not line.startswith(('TEL:', 'FAX:', 'PHONE:', 'ATTN:')):
                info_lines.append(line)
                if len(info_lines) >= max_lines:
                    break
    
    # Clean and join the collected lines
    cleaned_info = ' '.join(info_lines)
    return clean_text(cleaned_info)

def parse_bill_of_lading_fields(ocr_text, page_response):
    """Improved Bill of Lading parser that handles multiple formats"""
    lines = [line.strip() for line in ocr_text.split('\n') if line.strip()]
    n = len(lines)
    
    def find_field(label_patterns, value_pattern=None, max_lines_after=3, max_chars=200):
        """Find field value after label patterns"""
        for i, line in enumerate(lines):
            for pattern in label_patterns:
                if re.search(pattern, line, re.IGNORECASE):
                    # Look in subsequent lines for the value
                    for j in range(i+1, min(i+1+max_lines_after, n)):
    ...
    # Shipper with multiple possible label patterns
    shipper = extract_company_info(ocr_text, 
                                  ["EXPORTER", "SHIPPER", "PRINCIPAL", "SELLER"],
                                  ["CONSIGN", "NOTIFY", "FORWARDING", "DOCUMENT"])
    
    # Consignee with multiple possible label patterns
    consignee = extract_company_info(ocr_text,
                                    ["CONSIGN", "RECEIVER", "BUYER"],
                                    ["NOTIFY", "FORWARDING", "DOCUMENT"])
    
    # Port of Loading - multiple possible labels
    port_of_loading = find_field([
    ...
    # Port of Discharge - multiple possible labels
    port_of_discharge = find_field([
        r"PORT\s*OF\s*DISCHARGE",
        r"DISCHARGE\s*PORT",
        r"POD",
        r"PLACE\s*OF\s*DELIVERY",
        r"FOREIGN\s*PORT\s*OF\s*UNLOADING"
    ])
    
    # BL Number - multiple patterns
    bl_number = find_field([
        r"B\/L\s*NUMBER",
        r"DOCUMENT\s*NUMBER",
        r"BILL\s*OF\s*LADING\s*NO"
    ], r'\b[A-Z]{2,}[0-9]{6,}\b') or \
    re.search(r'\b(?:B\/L|BL)?\s*NO\.?\s*[:]?\s*([A-Z]{2,}[0-9]{6,})\b', 
              ocr_text, re.IGNORECASE)
    bl_number = bl_number.group(1) if bl_number else ""
    
    # Container Numbers - more comprehensive pattern
    container_numbers = set()
    container_matches = re.finditer(
        r'\b([A-Z]{3}[UJZ]\d{7}|[A-Z]{4}\d{7}|[A-Z]{3}\d{6})\b', 
        ocr_text)
    for match in container_matches:
        container_numbers.add(match.group(1))
    container_numbers = ', '.join(container_numbers) if container_numbers else ""
    
    # Vessel/Flight - multiple patterns
    flight_or_vessel = find_field([
        r"VESSEL\s*NAME",
        r"EXPORTING\s*CARRIER",
        r"OCEAN\s*VESSEL",
        r"FLIGHT\s*NUMBER"
    ])
    
    # Product Description - improved multi-line extraction
    product_description = ""
    desc_start = -1
    for i, line in enumerate(lines):
        if re.search(r'(DESCRIPTION\s*OF\s*GOODS|COMMODITY\s*DESCRIPTION|NATURE\s*OF\s*GOODS)', 
                    line, re.IGNORECASE):
            desc_start = i
            break
    ...
    if desc_start >= 0:
        desc_lines = []
        for j in range(desc_start+1, min(desc_start+10, n)):
            line = lines[j].strip()
            if not line or re.match(r'^(FREIGHT|CHARGE|WEIGHT|MEASURE)', line, re.IGNORECASE):
                break
            desc_lines.append(line)
        product_description = ' '.join(desc_lines)
...
def parse_air_waybill_fields(ocr_text, page_response):
    """Improved Air Waybill parser"""
    lines = [line.strip() for line in ocr_text.split('\n') if line.strip()]
    
    # AWB Number - more flexible pattern
    awb_number = re.search(r'\b(\d{3}[- ]?\d{7,8})\b', ocr_text)
    awb_number = awb_number.group(1) if awb_number else ""
    
    # Shipper - improved extraction
    shipper = extract_company_info(ocr_text, 
                                  ["SHIPPER", "SENDER"],
                                  ["CONSIGNEE", "NOTIFY"])
    
    # Consignee - improved extraction
    consignee = extract_company_info(ocr_text,
                                    ["CONSIGNEE", "RECEIVER"],
                                    ["NOTIFY", "ISSUING CARRIER"])
    
    # Port of Loading - multiple patterns
    port_of_loading = ""
    for i, line in enumerate(lines):
        if re.search(r'AIRPORT\s*OF\s*DEPARTURE|ORIGIN\s*AIRPORT', line, re.IGNORECASE):
            port_of_loading = lines[i+1] if i+1 < len(lines) else ""
            break
    
    # Port of Discharge - multiple patterns
    port_of_discharge = ""
    for i, line in enumerate(lines):
        if re.search(r'AIRPORT\s*OF\s*DESTINATION|DESTINATION\s*AIRPORT', line, re.IGNORECASE):
            port_of_discharge = lines[i+1] if i+1 < len(lines) else ""
            break
    
    # Flight info - improved
    flight_or_vessel = ""
    for i, line in enumerate(lines):
        if re.search(r'FLIGHT\s*NUMBER|CARRIER\s*FLIGHT', line, re.IGNORECASE):
            flight_or_vessel = lines[i+1] if i+1 < len(lines) else ""
            break
    
    # Package count - improved
    pkg_match = re.search(r'(\d{1,4})\s*(?:PCS|PKGS|PIECES|PACKAGES)\b', ocr_text, re.IGNORECASE)
    container_numbers = pkg_match.group(1) if pkg_match else ""
    
    # Product Description - improved
    product_description = ""
    for i, line in enumerate(lines):
        if re.search(r'NATURE\s*AND\s*QUANTITY|DESCRIPTION\s*OF\s*GOODS', line, re.IGNORECASE):
            for j in range(i+1, min(i+6, len(lines))):
                val = lines[j].strip()
                if val and not val.lower().startswith(('freight', 'charge', 'weight')):
...
def extract_fields(file_path):
    try:
        ...
        # Determine document type with more patterns
        if (re.search(r'AIR\s*WAYBILL|AWB\s*NUMBER', all_text, re.IGNORECASE) or 
            re.search(r'\b\d{3}[- ]?\d{7,8}\b', all_text)):
