...
def handle_payment_webhook():
    ...
    try:
        ...
        if not bill:
            logger.error(f"No bill found for transaction_id {transaction_id}")
        ...
        received_85 = abs(amount - (invoice_total * 0.85)) < 0.01  # Check if amount is ~85%
        received_15 = abs(amount - (invoice_total * 0.15)) < 0.01  # Check if amount is ~15%
        ...
        update_query += " WHERE bl_number = %s"
