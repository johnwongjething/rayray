...
def get_bills():
    ...
    if date:
        ...
        # When no date is selected, show all bills but don't calculate summary
        summary_data = {
    ...
    print(f"Summary stats: {summary_data}")
...
def get_awaiting_bank_in_bills():
    """Get only bills with 'Awaiting Bank In' status for AccountingReview page"""
    conn = get_db_conn()
    cur = conn.cursor()
